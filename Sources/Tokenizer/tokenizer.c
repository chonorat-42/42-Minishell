...
void	split_on_pipes(t_mshell *shell, char *str)
{
	...
	while (str[++i])
	{
		...
		else if (is_char_in_set(str[i], "|"))
		{
			...
			{
				ft_dprintf(2, "minishell: syntax error near unexpected token `|'\n");
				return (free_tokens(&shell->tok_lst), free(shell->input), free_arr(shell->paths), shell->paths = NULL, get_input_loop(shell));
			}
...
int	tokenizer(t_mshell *shell)
{
	...
	if (!shell->tok_lst || !shell->tok_lst->cmd_arr
		...
		return (close_fd(shell), free_tokens(&shell->tok_lst), free(shell->input),
